...
public class OpenGLEngine {
	...
	public static void drawWindowIn() throws Exception{
		...
		Texture liveTexture = BufferedImageUtil.getTexture("liveWindowTexture", windowCatcher.getNewFrame());
		//System.out.println(liveTexture.getImageHeight());
	    glBindTexture(GL_TEXTURE_2D, liveTexture.getTextureID());
	   
	    glBegin(GL_QUADS); //liveTexture.getImageHeight() liveTexture.getImageWidth()
	        glTexCoord2d(0.0, 0.0); glVertex2i(0,liveTexture.getImageHeight());
	        glTexCoord2d(1.0, 0.0); glVertex2i(liveTexture.getImageWidth(),liveTexture.getImageHeight());
	        glTexCoord2d(1.0, 1.0); glVertex2i(liveTexture.getImageWidth(),0);
